...
from PySide6.QtWidgets import (
    ...
    QGroupBox, QComboBox, QScrollArea, QDialog, QApplication, QCheckBox, QDoubleSpinBox,
    QDateEdit,
    QDateTimeEdit,
    QPlainTextEdit
)
...
# ===== BEGIN: robust integer parsing helpers (auto-injected) =====
from decimal import Decimal, InvalidOperation
import re as _re_for_intparse

_PERS_AR_DIGIT_TRANS = str.maketrans("۰۱۲۳۴۵۶۷۸۹٠١٢٣٤٥٦٧٨٩","01234567890123456789")
_THOUSAND_SEPS = (",", "٬", "،", " ")

def _normalize_numeric_string__auto(s: str) -> str:
    s = s.strip().translate(_PERS_AR_DIGIT_TRANS)
    for _sep in _THOUSAND_SEPS:
        s = s.replace(_sep, "")
    return s

def parse_int_str__auto(s: str, *, allow_scientific: bool = True, min_val: int | None = None, max_val: int | None = None) -> int:
    if s is None:
        raise ValueError("ورودی خالی است.")
    raw = s
    s = _normalize_numeric_string__auto(s)
    if not s:
        raise ValueError("ورودی خالی است.")
    if not allow_scientific and _re_for_intparse.search(r"[eE]", s):
        raise ValueError("نمای علمی (مثل 1E+15) مجاز نیست.")
    try:
        d = Decimal(s)
    except InvalidOperation:
        raise ValueError(f"مقدار عددی معتبر نیست: «{raw}».")
    if d != d.to_integral_value():
        raise ValueError("مقدار باید عدد صحیح بدون اعشار باشد.")
    val = int(d)
    if min_val is not None and val < min_val:
        raise ValueError(f"کمتر از حداقل مجاز ({min_val}).")
    if max_val is not None and val > max_val:
        raise ValueError(f"بیشتر از حداکثر مجاز ({max_val}).")
    return val
# ===== END: robust integer parsing helpers (auto-injected) =====
...
def upsert_record_to_sheet(rec: dict,
    ...
    try:
        # تشخیص اینکه واقعاً همین فایل اجرا می‌شود
        # print("UPSERT from:", __file__)

        if status_label:
            status_label.setText("در حال اتصال به Google Sheets…")
...
"https://www.googleapis.com/auth/spreadsheets",
        ...
        # header
        if status_label:
            status_label.setText("همگام‌سازی هدر شیت…")
        ...
        # مهم: trim هدرها تا mismatch نخوریم
        cur_header = [str(h).strip() for h in cur_header]
        ...
        if not rec_id:
            raise ValueError("شناسهٔ رکورد (id) موجود نیست.")
        ...
        # id→row
        if status_label:
            status_label.setText("جستجوی سطر مربوط به id…")
        ...
        # helper: مقدار را به لیست تبدیل کن (پشتیبانی خواندن قدیمی‌های ‘|’ فقط برای read)
        def to_list(x):
            if x is None:
                return []
        ...
        if rec_id in id_to_row:
            # UPDATE: ادغام با مقدار قبلی و نوشتن JSON
            r = id_to_row[rec_id]
            for k in merge_keys:
                if k in cur_header:
                    ...
                    row_values[idx] = json.dumps(merged_list, ensure_ascii=False)
        else:
            # INSERT: اگر لیست دادی، JSON بنویس
            for k in merge_keys:
                if k in cur_header:
                    idx = cur_header.index(k)
        ...
        if rec_id in id_to_row:
            ...
            if status_label:
                status_label.setText("به‌روزرسانی سطر موجود…")
            backoff(sh.values_batch_update, body)
            return "updated"
        else:
            ...
            if status_label:
                status_label.setText("افزودن سطر جدید…")
...
"https://www.googleapis.com/auth/spreadsheets",
    ...
    for key in ("notes","reminder notes","reminder_notes","report_uploaded_files","uploaded_files","petition_uploaded_files"):
        if key in rec:
...
class EditorWindow(QTabWidget):
    ...
    def _init_ui(self):
        ...
        # واکنش به تغییر نوع پرونده برای کنترل فیلدهای مبلغ مالیات و جریمه‌ها
        ...
        # فیلدهای دوره‌ای مخصوص پرونده‌های «ارزش افزوده» (۴ دوره)
        self.vat_period_rows = []
        _period_labels = ["اول", "دوم", "سوم", "چهارم"]
        for _idx_p, _label_p in enumerate(_period_labels, start=1):
            row_w = QWidget()
            h = QHBoxLayout(row_w)
            ...
            form.addRow(f"دوره {_label_p}:", row_w)
...
# آپلود فایل‌های گزارش رسیدگی
    ...
    def _on_penalty_amount_edited(self, _text: str):
        """هندلر فرمت زنده برای QLineEdit های عددی (مبلغ مالیات/جریمه و دوره‌ها)."""
    ...
    def _on_assessment_changed(self, id):
        ...
        if has_ass:
            ...
            for rb in (self.rb_report_yes, self.rb_report_no,
                ...
                rb.setEnabled(True)
            # فعال‌سازی اولیه؛ جزئیات توسط _update_tax_amount_state کنترل می‌شود
            self.penalty_amount.setEnabled(True)
        else:
        ...
        # وضعیت فیلدهای مالیات/جریمه را بر اساس نوع پرونده و برگه تشخیص به‌روزرسانی کن
        try:
            self._update_tax_amount_state()
        except AttributeError:
            # ممکن است قبل از ساخته‌شدن ویجت‌ها صدا زده شود
            pass
    ...
    def _update_tax_amount_state(self):
        """به‌روزرسانی وضعیت فیلدهای مبلغ مالیات و جریمه‌ها بر اساس نوع پرونده و داشتن برگه تشخیص."""
        # اگر ویجت‌های مربوطه هنوز ساخته نشده‌اند، کاری نکن
        if not hasattr(self, "penalty_amount"):
            return

        selected_cases = [cb.text() for cb in getattr(self, 'case_checkboxes', []) if cb.isChecked()]
        has_ass = hasattr(self, "group_has_ass") and (self.group_has_ass.checkedId() == 1)

        if not selected_cases:
            # هیچ نوع پرونده‌ای انتخاب نشده
            self.penalty_amount.clear()
            self.penalty_amount.setEnabled(False)
            # پنهان‌کردن و پاک‌کردن فیلدهای تکمیلی
        ...
        only_vat = (len(selected_cases) == 1 and is_vat)

        # فیلد مبلغ مالیات کلی:
        # در حالتی که فقط ارزش افزوده است، این فیلد استفاده نمی‌شود
        if only_vat:
            self.penalty_amount.clear()
            self.penalty_amount.setEnabled(False)
        else:
            self.penalty_amount.setEnabled(bool(has_ass))
        ...
        # فیلدهای دوره‌ای مخصوص ارزش افزوده
        if hasattr(self, "vat_period_rows"):
            for row_w, tax_edit, pen_edit in self.vat_period_rows:
    ...
    def _on_case_type_changed(self, _checked: bool):
        """هر تغییری در نوع پرونده روی وضعیت فیلدهای مالیات و جریمه اثر می‌گذارد."""
        try:
            self._update_tax_amount_state()
        except AttributeError:
            # اگر هنوز ویجت‌ها ساخته نشده‌اند
            pass
        self._mark_dirty()
    ...
    def _snapshot_current_form(self) -> Dict[str, Any]:
        ...
        mad238_res = "تعدیل مالیات" if self.rb_adjust.isChecked() else ("ارجاع به هیأت بدوی" if self.rb_refer.isChecked() else "")
        jarime_amount = self.penalty_amount.text().strip()

        # جریمه‌ی مخصوص «مالیات بر عملکرد»
        performance_penalty = ""
        if hasattr(self, "performance_penalty"):
            performance_penalty = self.performance_penalty.text().strip()

        # دوره‌های ارزش افزوده
        vat_fields: Dict[str, Any] = {}
        if hasattr(self, "vat_period_rows") and self.vat_period_rows:
            for idx, (_row_w, tax_edit, pen_edit) in enumerate(self.vat_period_rows, start=1):
                tax_txt = tax_edit.text().strip()
                pen_txt = pen_edit.text().strip()
                vat_fields[f"vat_p{idx}_tax"] = tax_txt
                vat_fields[f"vat_p{idx}_penalty"] = pen_txt
        else:
            # اگر در این نسخه فیلدها را نداشته باشی، خالی نگه‌دار
        ...
        base = {
            ...
            'vat_p1_tax': vat_fields['vat_p1_tax'],
            'vat_p1_penalty': vat_fields['vat_p1_penalty'],
            'vat_p2_tax': vat_fields['vat_p2_tax'],
            'vat_p2_penalty': vat_fields['vat_p2_penalty'],
            'vat_p3_tax': vat_fields['vat_p3_tax'],
            'vat_p3_penalty': vat_fields['vat_p3_penalty'],
            'vat_p4_tax': vat_fields['vat_p4_tax'],
            'vat_p4_penalty': vat_fields['vat_p4_penalty'],
    ...
    def _mark_dirty(self):
        ...
        orig = getattr(self, "_orig_snapshot", None)
        self.btn_save.setEnabled(orig is not None and cur != orig)
    ...
    def _wire_dirty_watchers(self):
        ...
        for g in [self.group_case, self.group_status, self.group_has_ass, self.group_report, self.group_objected]:
            for b in g.buttons():
    ...
    def _load_record_into_form(self, idx: int):
        ...
        try:
            ...
            pass
        # اسنپ‌شات اولیه فرم برای کنترل دکمه ذخیره
    ...
    def _save(self):
        ...
        # تشخیص این‌که فقط «ارزش افزوده» به‌عنوان نوع پرونده انتخاب شده است یا خیر
    ...
    def _finalize(self, full_record: dict):
        ...
        try:
            ...
            # همگام‌سازی اسنپ‌شات پس از ذخیره موفق
            self._orig_snapshot = json.dumps(self._snapshot_current_form(), ensure_ascii=False, sort_keys=True)
            self._mark_dirty()
...
def main():
    ...
    ap.add_argument("--sheet-id", type=str, default=os.environ.get("GOOGLE_SHEETS_KEY"))
    ap.add_argument("--sheet-name", type=str, default=os.environ.get("GOOGLE_SHEET_NAME", "Sheet1"))
